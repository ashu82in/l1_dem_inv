...
# ------------------------------------------------
# 4. Simulation Engine (Physical Trigger Logic)
# ------------------------------------------------
def run_sim(q_val, d_seq, d_dates):
    ...
    for day in range(len(d_seq)):
        # 1. Receipt of goods at START of day
        ...
        # 2. TRIGGER CHECK: Based strictly on Physical Inventory AFTER arrivals
        ...
        is_unfulfilled = daily_demand > (opening + received)
        
        # Final State for today
        final_pos = inv + sum(o[1] for o in pipeline)
        ...
        rows.append({
            ...
            "Inventory Position": int(final_pos),
...
with tab1:
    ...
    m3.metric("LT Fill Rate", f"{lt_fr:.1f}%", help="Fill rate during lead time cycles")
    ...
    if not trigger_pts.empty:
        fig_inv.add_trace(go.Scatter(
            x=trigger_pts["Date"], y=trigger_pts["Physical Inventory"], mode="markers", name="Order Placed", 
    ...
    shortage_pts = df[df["Is Stockout"] == True]
    if not shortage_pts.empty:
        fig_inv.add_trace(go.Scatter(x=shortage_pts["Date"], y=shortage_pts["Physical Inventory"], mode="markers", name="Shortage", marker=dict(color="red", size=10, symbol="x")))
    ...
    with cr: st.plotly_chart(px.histogram(df, x="Demand", nbins=20, title="Daily Distribution", color_discrete_sequence=['#00CC96']).update_layout(template="plotly_dark", height=300, bargap=0.1), use_container_width=True)
...
with tab2:
    ...
    with h_col: hist_mode = st.radio("Select Distribution:", ["Daily Demand", f"{window_size}-Day Window Demand"])
    ...
    r2.metric(f"Absolute Max", int(max_val))
    r3.metric("Uncovered Risk Gap", int(max_val - cutoff), delta="Exposure", delta_color="inverse")
