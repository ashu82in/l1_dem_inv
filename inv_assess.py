...
if "demand_seq" not in st.session_state or st.session_state.get("last_key") != demand_state_key or regen_button:
    st.session_state.last_key = demand_state_key
    if cov <= 0:
...
# ------------------------------------------------
# 4. Simulation Engine (Corrected Timing: Position <= ROP)
# ------------------------------------------------
def run_sim(q_val, d_seq, d_dates):
    ...
    for day in range(len(d_seq)):
        ...
        # 3. Position Check BEFORE trigger
        current_pipeline_qty = sum(o[1] for o in pipeline)
        inv_pos_at_check = inv + current_pipeline_qty
        
        # 4. Trigger Order (Position <= ROP)
        placed_qty = 0
        if inv_pos_at_check <= reorder_point:
            placed_qty = q_val
            pipeline.append((day + lead_time, placed_qty))
        
        rows.append({
            "Date": d_dates[day].date(), "Opening": int(opening), "Demand": int(daily_demand), 
            "Shortage": int(shortage), "Is Stockout": is_unfulfilled, "In Lead Time": len(pipeline) > 0, 
...
with tab1:
    ...
    m3.metric("LT Fill Rate", f"{lt_fr:.1f}%")
    ...
    # 3. 1-Day Demand Visuals
    st.divider()
    st.subheader("Daily Demand Overview")
    c_line, c_hist = st.columns(2)
    with c_line:
        fig_daily_line = px.line(df, x="Date", y="Demand", title="Daily Volatility", color_discrete_sequence=['#AB63FA'])
        fig_daily_line.update_layout(template="plotly_dark", height=350)
        st.plotly_chart(fig_daily_line, use_container_width=True)
    with c_hist:
        fig_daily_hist = px.histogram(df, x="Demand", nbins=20, title="Daily Distribution", color_discrete_sequence=['#00CC96'], marginal="box")
...
with tab2:
    ...
    fig_roll.update_layout(template="plotly_dark", height=400, hovermode="x unified")
    ...
    # 3. Dynamic Histogram & Risk Exposure (WINDOW BASED)
